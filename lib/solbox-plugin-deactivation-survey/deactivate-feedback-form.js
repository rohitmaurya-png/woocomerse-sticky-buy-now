...
let solbox = {};
		...
		var element = $('\
			<div class="solbox-deactivate-dialog" data-remodal-id="' + plugin.slug + '">\
				<form>\
					...
					<input type="hidden" name="action" value="wsatc_deactivation_feedback" />\
	...
	$(document).ready(function() {
		...
		for(var i = 0; i < solbox_deactivate_feedback_form_wsatc.length; i++)
		{
			var plugin = solbox_deactivate_feedback_form_wsatc[i];
